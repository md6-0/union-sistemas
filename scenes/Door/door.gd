...
func interact(_camera):
	if required_key == "" or GameManager.inventory.get(required_key, 0) > 0:
		...
		tween.tween_property(self, "rotation:y", deg_to_rad(target), 1.5).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		tween.finished.connect(_handle_door_state)
		
		audioStreamPlayer_open.pitch_scale = randf_range(0.9, 1.1)
